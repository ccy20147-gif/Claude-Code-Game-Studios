...
`scripts/run_ue_automation.py` and `scripts/build_ue_package.py` are fixed-argument local execution wrappers, not MCP routes or capability activation. They default to an execution plan and require `--execute`; the former only permits `Project.Smoke.*` and `Project.Automation.*` filters, while the latter only permits Win64 Shipping BuildCookRun into a new or empty staging directory. Both write normalized runtime evidence, record timeouts as `UNKNOWN`, and never retry automatically. A successful run does not change `catalog/capabilities.yaml`: validate independent Windows UE 5.7.4 automation, package, clean-install launch, and smoke evidence before a later approved availability change.

`templates/blueprint-operation.yaml` and `templates/dcc-operation.yaml` are similarly fail-closed contracts. Blueprint operations have action-specific template, component, variable, and graph-patch allowlists; graph patches require an approval record. DCC operations must stay below the project's `dcc/` workspace, with modify/delete requiring approval. Their validators reject `READY` while their catalog capabilities are `BLOCKED`; a server-side policy, checksum-locked integration, and independent save/reload or Blender round-trip canary remain required.
