...
When the graph passes structural validation, still require a spoiler-safe fresh-tester playtest. Store a tester export without truth or solution nodes. A clue or line revision must flow through `$ue5-change-design` and invalidate only its dependent hypotheses, level placements, media, and tests.
