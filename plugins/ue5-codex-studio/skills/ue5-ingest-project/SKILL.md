...
Route fiction to `$ue5-adapt-story`. Route every design pack, design link, or hybrid design input through `$ue5-reconcile-gdd` before `$ue5-accept-baseline`; use `$ue5-map-systems` only after the reconciled GDD is accepted. An intake bundle is evidence for review, not an accepted design baseline.
